...
def _find_player(db: Session, player_name: str, game: Optional[Game] = None) -> Optional[Player]:
    ...
    query = db.query(Player).filter(Player.full_name == clean_name)
    
    if game:
        team_ids = [game.home_team_id, game.away_team_id]
        p = query.filter(Player.team_id.in_(team_ids)).first()
        if p:
            return p

    # Next try active offensive players
    p = query.filter(Player.status == "ACT", Player.position.in_(["QB", "RB", "WR", "TE", "K"])).first()
    if p:
        return p
