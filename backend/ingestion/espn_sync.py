                    ...
                    for group in dc_data.get('depthchart', []):
                        ...
                        for pos_k, pos_v in positions.items():
                            ...
                            for rank_idx, ath in enumerate(athletes, start=1):
                                ...
                                # Match player in DB
                                player = db.query(Player).filter(Player.full_name == ath_name).first()
                                if not player:
                                    # Try fuzzy or create
                                    player = Player(
                    ...
                    for cat in r_data.get('athletes', []):
                        ...
                        if cat_pos == 'injuredReserveOrOut':
                            for ath in cat.get('items', []):
                                p = db.query(Player).filter(Player.full_name == ath.get('displayName')).first()
                                if p:
                                    p.status = 'RES'
                            ...
                            for ath in cat.get('items', []):
                                ...
                                if p_pos in ['QB', 'RB', 'WR', 'TE', 'PK', 'K']:
                                    ...
                                    p = db.query(Player).filter(Player.full_name == p_name).first()
                                    if p:
